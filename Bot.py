...
# استكمال الكود من الخطوة السابقة
...
async def handle_second_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    photo = await update.message.photo[-1].get_file()
    path = f"dual2_{update.effective_user.id}_{int(time.time())}.jpg"
...
3. التحقق من التوافق: التأكد من تطابق الاتجاه والإشارات
4. إصدار التوصية النهائية: شراء/بيع/انتظار
...
• سبب القرار: [توضيح بناءً على التوافق بين الفريمين]
...
"""
            ...
            if response.status_code == 200:
                ...
                # حفظ سياق التحليل
                save_analysis_context(user_id, analysis)
                
            else:
                await wait_msg.edit_text(f"❌ حدث خطأ في التحليل المزدوج. الرمز: {response.status_code}")
