...
1. **المدرسة المعتمدة:** SMC و ICT كإطار عمل رئيسي مع دعم بالتحليل الكلاسيكي
...
🛡️ طبقات الحماية من الانعكاس الفاشل (Protection Layers):
- فلتر ضد القطار السريع: يمنع الدخول عكس الاتجاه لمجرد وصول السعر لـ POI، لا دخول إلا بعد شمعة ابتلاعية عكسية ≥100% من جسم الشمعة السابقة
- تأكيد البكسل للكسر: لا BOS إلا إذا أغلق جسم الشمعة بالكامل خلف المستوى، لمس الذيل فقط = Liquidity Sweep
- تباعد الحجم والسعر: صعود بأجسام شموع تتناقص تدريجياً = زخم وهمي، رفع الحذر من V-Reversal
- قاعدة الانتظار الزمني: في OTC، انتظر إغلاق شمعتين بعد ملامسة POI قبل إصدار الإشارة النهائية
- فلتر السيولة الزمنية: يمنع الدخول خارج جلسات السيولة العالية أو آخر 15 دقيقة إلا عند وجود كسر هيكلي واضح + تأكيد 3 شموع
- فلتر التوازن السعري: يمنع الدخول عندما يكون السعر قريباً من خط التوازن 50% بدون كسر هيكلي أو امتصاص سيولة واضح
- فلتر التشبع السعري: يمنع الدخول بعد 3 شموع كبيرة متتالية بنفس الاتجاه بدون تصحيح أو إعادة اختبار
- فلتر الانعكاس الهيكلي الزائف: يمنع اعتبار CHoCh أو BOS صحيحاً إلا إذا تزامن مع سحب سيولة واضح + إغلاق جسم شمعة خلف الهيكل + متابعة بشمعة ثانية
- ربط الفلاتر بالقرار النهائي: أي فلتر مفعل يمنع القرار النهائي حتى تحقق شرطه
- توافق الإطارات: لا تنفيذ إلا عند توافق 3/4+ أو HTF متوافق مع POI وOB
- تأكيد RR النهائي: RR ≥ 1:2 بعد مراجعة تأثير الأخبار والتقلبات
...
⚠️ **قواعد تثبيت القرار النهائي**
1. اجعل القرار النهائي **ثابت وموحد** لكل صورة متطابقة
2. لا تغيّر القرار بين التحليلات المتكررة إلا عند تغيير واضح في الشارت
3. استخدم **تأكيد متعدد المرات**: القرار يعتبر صحيحاً إذا ظهر مرتين على الأقل بنفس المعطيات
...
**ملاحظة:** استخدم الصورة للتحقق من كل الأرقام والمستويات.
...
• التحليل المقترح: {initial_analysis}  
• استراتيجية التداول: {trading_strategy}  
...
1. دقة الأرقام: مطابقة كل سعر مع البكسل الفعلي للشارت  
2. سلامة المنطق: عدم وجود تناقضات  
3. الالتزام بالتنسيق  
4. تحسين الصياغة ووضوح اللغة  
5. إضافة أي رؤى إضافية لم تذكر  

📊 **المرحلة 1: الفحص الأولي والتحذيرات**
#1.1 نظام الأمان ثلاثي الطبقات
• الدرع الأساسي: {news_warning if news_warning else "✅ الوضع آمن"}  
• كشف وهم الزخم: 3 شموع متتالية + فحص المتابعة  
• التحقق الرقمي: استخراج الأسعار بدقة من المحور  

#1.2 كشف مخاطر OTC
• إشارات التلاعب: اختراق ثم عودة، انعكاس لحظي، حركة بدون حجم  
• إستراتيجية الحماية: تجنب آخر 10 ثوانٍ، أوامر معلقة، SL موسع +20%  
• تنبيه OTC: تحقق من أي تحركات خوارزمية قبل أي قرار دخول  
...
#2.2 الإحداثيات الرقمية
• أعلى سعر (High), أدنى سعر (Low), نقطة المنتصف (50%), نسبة الحركة  

#2.3 مصفاة التسعير
• Discount: شراء فقط, Premium: بيع فقط  
• الطوارئ: أسفل 20% / أعلى 80% — يمنع الدخول  

💰 **المرحلة 3: السيولة والزخم**
#3.1 خرائط السيولة
• Equal Highs/Lows, Inducement Zones, Liquidity Sweeps, FVG المفتوحة  

#3.2 كشف وهم الزخم
• شمعة خبر منفردة, فجوات سعرية, ذيول طويلة, V-Reversal  

#3.3 انعكاس الزخم
• رفض بعد اندفاع, فشل اختراق, انخفاض حجم, دايفرجنس عند POI  

🛡️ **طبقات الحماية من الانعكاس**
- فلتر ضد القطار السريع: لا دخول عكس الاتجاه إلا بعد شمعة ابتلاعية ≥100%  
- تأكيد البكسل للكسر: لا BOS إلا إذا أغلق الجسم بالكامل، لمس الذيل فقط = Liquidity Sweep  
- تباعد الحجم والسعر: صعود بأجسام تصغر تدريجياً = زخم وهمي  
- الانتظار الزمني: في OTC، إغلاق شمعتين بعد ملامسة POI قبل القرار النهائي  
- فلتر السيولة الزمنية: يمنع الدخول خارج جلسات السيولة العالية أو آخر 15 دقيقة إلا مع كسر هيكلي + 3 شموع تأكيد  
- فلتر التوازن السعري: يمنع الدخول بالقرب من خط 50% بدون كسر هيكلي أو امتصاص واضح  
- فلتر التشبع السعري: يمنع الدخول بعد 3 شموع كبيرة متتالية بنفس الاتجاه بدون تصحيح  
- فلتر الانعكاس الهيكلي الزائف: CHoCh صحيح فقط إذا تزامن مع سحب سيولة واضح + إغلاق جسم شمعة خلف الهيكل + متابعة شمعة ثانية  
- ربط الفلاتر بالقرار النهائي: أي فلتر مفعل يمنع القرار النهائي حتى تحقق شرطه  
- توافق الإطارات قبل التنفيذ: لا تنفيذ إلا عند توافق 3/4+ أو HTF متوافق مع OB + POI  
- تأكيد RR النهائي: RR ≥ 1:2 بعد تعديل الأخبار  

🎯 **المرحلة 4: القرار الذكي**
✔️ POI صالح, نموذج شموعي واضح, سلوك سعري متوافق, توافق مع الاتجاه العام  
❌ فشل أي بند → لا دخول  
...
• HTF: الاتجاه العام, MTF1: مناطق العرض/الطلب, MTF2: OB النشطة, LTF: توقيت الدخول  

**توافق الإطارات:** 4/4 → +40, 3/4 → +30, 2/4 → تقليل الحجم 50%, 1/4 → منع الدخول  

🎯 **المرحلة 7: درجات الثقة**
**إضافات (+):** POI +25, نموذج شموعي +20, سلوك سعري +25, توافق 3/4+ +30, BOS +30, حجم أعلى +15, أخبار هادئة +20, توافق MACD +10  
**خصومات (-):** تعارض مؤشرات -20, أخبار قوية -25, زخم وهمي -15, V-Reversal قريب -30, سيولة OTC منخفضة -10  
**مستوى الثقة:** [0-100] → [💥/🔥/⚡/❄️/🚫]  

📊 **المرحلة 8: الحجم المتقدم**
• اختراق >150%, امتصاص: حجم مرتفع + حركة محدودة, تصحيح <70%  
• POC = دعم/مقاومة, EVA خارج VA = إشارة قوة  

🔄 **المرحلة 9: إدارة الصفقة**
Long: TP1 SL للتعادل +40%, TP2 SL أعلى آخر شمعة +30%, TP3 ترك 30% بترايل  
Short: TP1 SL للتعادل +40%, TP2 SL أسفل آخر شمعة +30%, TP3 ترك 30% بترايل  
OTC حماية: SL موسع +20%, دخول بعد 3 شموع، حجم متدرج 33/33/34  

🧠 **المرحلة 10: السلوك والتلاعب**
• Liquidity Sweep, Stop Hunt, False Breakout, Bait Pattern  
• التمييز: اختراق بذيل + عودة = فخ, اختراق جسم كامل + إغلاق خلف المستوى = BOS حقيقي
