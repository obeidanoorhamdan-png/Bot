...
# --- كود تحليل الصور المحسن والمدمج الكامل مع نظام الموديل الثلاثي ---
async def handle_photo_analysis(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالجة الصور للتحليل الفني المتقدم مع نظام الموديل الثلاثي المتوازي"""
    user_id = update.effective_user.id
    ...
    wait_msg = await update.message.reply_text("📊 جاري تحليل شارت بتقنيات ثلاثية النماذج ... ⚡")
...
"""
        ...
        # --- النظام الثلاثي: تحليل متوازي بـ 3 نماذج في نفس الوقت ---
        await wait_msg.edit_text("📊 جاري التحليل (النظام الثلاثي 1/3)... ⚡")
        ...
        models = {
            ...
            "mistral_ocr": "mistral-ocr-latest"  # موديل OCR للقراءة الدقيقة
        }
        
        # تحضير الطلبات للنماذج الثلاثة في نفس الوقت
        payloads = {
            "pixtral_large": {
                ...
                "messages": [
                    {
                        "role": "user", 
                        "content": [
                            {"type": "text", "text": f"{prompt}\n\n🎯 **التركيز الرئيسي:** التحليل الهيكلي المتقدم والسلوك السعري."},
                ...
                "messages": [
                    {
                        "role": "user", 
                        "content": [
                            {"type": "text", "text": f"{prompt}\n\n🎯 **التركيز الرئيسي:** التدقيق الدقيق والتحليل الاحترافي للجودة العالية."},
                            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64_img}", "detail": "high"}}
                        ]
                    }
                ],
            ...
            "mistral_ocr": {
                "model": models["mistral_ocr"],
                "messages": [
                    {
                        "role": "user", 
                        "content": [
                            {"type": "text", "text": f"""أنت خبير في قراءة واستخراج البيانات من الشارتات المالية.
                            
                            مهمتك الأساسية: استخراج جميع البيانات والأرقام بدقة من الشارت المرفق.
                            
                            🔍 **ما يجب استخراجه:**
                            1. **السعر الحالي:** الرقم الدقيق المعروض على المحور
                            2. **أعلى سعر:** أعلى نقطة مرئية في الشارت
                            3. **أقل سعر:** أدنى نقطة مرئية في الشارت
                            4. **مستويات الدعم:** أي مستويات دعم واضحة مع الأسعار
                            5. **مستويات المقاومة:** أي مستويات مقاومة واضحة مع الأسعار
                            6. **المؤشرات الفنية:** أي مؤشرات موجودة (RSI, MACD, etc.) مع قيمها
                            7. **الزمن:** الإطار الزمني الظاهر على الشارت
                            8. **العملة/السهم:** اسم الأصل المالي إذا كان مكتوباً
                            
                            🎯 **التنسيق المطلوب:**
                            📊 **بيانات الشارت المستخرجة:**
                            • السعر الحالي: [رقم]
                            • أعلى سعر اليوم/الجلسة: [رقم]
                            • أقل سعر اليوم/الجلسة: [رقم]
                            • الدعم الرئيسي: [رقم]
                            • المقاومة الرئيسية: [رقم]
                            • المؤشرات: [قائمة بالمؤشرات وقيمها]
                            • الإطار الزمني: [الزمن]
                            • الأصل المالي: [إذا كان مكتوباً]
                            
                            ⚠️ **المهم:**
                            - الدقة في قراءة الأرقام أولوية
                            - عدم التخمين، فقط ما تراه بوضوح
                            - الإشارة إذا كان هناك غموض في القراءة"""},
                            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64_img}", "detail": "high"}}
                        ]
                    }
                ],
        ...
        # إرسال الطلبات الثلاثة في نفس الوقت
        import asyncio
        ...
        def send_request(model_name, payload):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    return model_name, f"Error {response.status_code}: {response.text[:100]}", "error"
            except Exception as e:
        ...
        merge_prompt = f"""
أنت المدير التنفيذي للتحليل الفني في Obeida Trading، مسؤول عن دمج وتحسين وتحقيق الانسجام بين ثلاثة تحليلات مختلفة تم إنتاجها بواسطة أنظمة ذكاء اصطناعي متطورة.

🔗 **التحليلات الواردة:**

1. **تحليل OCR للبيانات الدقيقة:**
{ocr_data if ocr_data else "⚠️ لم يتم الحصول على بيانات OCR"}

2. **التحليلين الرئيسيين:**
{"".join(main_analyses) if main_analyses else "⚠️ لم يتم الحصول على تحليلات رئيسية"}

🎯 **مهمتك الأساسية:**
1. **استخراج الحقائق الثابتة:** اعتمد البيانات الدقيقة من تحليل OCR (الأرقام، المستويات، المؤشرات)
2. **دمج الرؤى:** ادمج أفضل الأفكار من التحليلين الرئيسيين، واحتفظ بالرؤى الفريدة من كل نموذج
3. **حل التعارضات:** إذا وجدت تضارباً بين التحليلات، اعتمد الأكثر تحفظاً والأكثر دقة بناءً على السياق
4. **التنظيم الاحترافي:** نظم التقرير النهائي بحيث يكون واضحاً، متسلسلاً، وغنياً بالمعلومات
5. **إضافة القيمة:** أضف أي استنتاجات ذكية أو تحذيرات إضافية يمكن استخلاصها من البيانات المجمعة

📊 **قواعد الدمج:**
• إذا وجد اختلاف في الأسعار → استخدم الأرقام من OCR
• إذا وجد اختلاف في التوصية → اختر التوصية الأكثر تحفظاً مع أعلى مستوى ثقة
• إذا وجد اختلاف في المستويات → اعتمد النطاق الأوسع
• احتفظ بجميع الأفكار الفريدة والمفيدة من جميع التحليلات
...
• **خريطة السيولة:** (أقرب فخ سيولة Inducement + مناطق السيولة المستهدفة)
• **الفجوات السعرية:** (المناطق التي سيعود السعر لتغطيتها)
• **البيانات الدقيقة:** [استخدم هنا البيانات من OCR]

🎯 **الإشارة التنفيذية (المدمجة):**
• **السعر الحالي:** [من OCR]
• **حالة الشمعة:** [مفتوحة / مغلقة]
• **القرار الفني (المدمج):** (شراء 🟢 / بيع 🔴 / احتفاظ 🟡)
• **قوة الإشارة (المجمعة):** (عالية جدا 💥 / عالية 🔥 / متوسطة ⚡ / ضعيفة ❄️)
• **نقطة الدخول (المدققة):** [السعر الدقيق بناءً على OB + شرط الإغلاق]
• **الأهداف الربحية (المحسوبة):**
  🎯 **TP1:** [سحب أول سيولة داخلية], [احتمالية الوصول]
  🎯 **TP2:** [الهدف الرئيسي - منطقة عرض/طلب قوية]
  🎯 **TP3:** [سيولة خارجية أو سد فجوة سعرية]
• **وقف الخسارة (المحصن):** [السعر مع 3 طبقات حماية]
• **المدة المتوقعة:** [عدد الدقائق] (بناءً على معادلة الزخم السعري)
• **وقت الذروة المتوقع:** [مثلاً: خلال الـ 3 شموع القادمة]
• **الحالة النفسية (المحللة):** [خوف 🥺 / جشع 🤑 / تردد 🤌 / استسلام 👎]
• **علامات التلاعب (المكشوفة):** [موجودة ✔️ / غير موجودة ❎]
...
"""
        ...
        # إرسال برومبت الدمج للنموذج الأساسي
        merge_payload = {
            "model": MISTRAL_MODEL,  # استخدام النموذج الأساسي للدمج
            "messages": [
        ...
        if merge_response.status_code == 200:
            final_result = merge_response.json()['choices'][0]['message']['content'].strip()
        else:
            # إذا فشل الدمج، استخدم أفضل تحليل متاح
            print(f"❌ Merge failed, using best available analysis")
            if analyses:
                # اختيار أطول تحليل كبديل
