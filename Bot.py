...
async def handle_photo_analysis(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        prompt = f"""
        [SYSTEM_TASK: TOTAL_MARKET_DECRYPTION_V5]
        بصفتك خبير استراتيجيات تداول في صناديق التحوط، ومتمكن من دمج مدارس (SMC + ICT + Wyckoff + Order Flow)، قم بتحليل الشارت المرفق بدقة متناهية:
             ⚠️ **قوانين التحليل الإجبارية:**
        1. التحقق من المحور السعري (Y-Axis): قبل البدء، حدد "عائلة الرقم" الصحيحة الظاهرة على يمين الشاشة (مثلاً 3631.xx).
        2. مطابقة السعر الحالي: اقرأ الرقم الموجود في المربع الملون (Price Label) وقارنه بالقمم والقيعان القريبة للتأكد من عدم وجود إزاحة بصرية.
        3. التدقيق في الفتائل: تعامل مع أطراف الظلال (Wicks) كأرقام دقيقة لمستويات السيولة.
        .4. **قانون السيولة:** لا تعترف بأي دخول ما لم يتم سحب سيولة (Liquidity Sweep) واضحة.
         5**قانون المناطق:** الدخول حصراً من مناطق Discount (شراء) أو Premium (بيع).
         6. **قانون TP3:** يجب أن يستهدف سيولة خارجية بعيدة (External Liquidity).
         7.**قانون الإلغاء:** أي إغلاق خلف الـ SL يلغي الفرضية فوراً.
        
       🔍 **مراحل الفحص الفني (The 5 Stages):**
📊 **المرحلة الأولى: تشريح السيولة (Liquidity Engineering)**
- ابحث عن Equal Highs/Lows. هل السعر يتجه لضرب ستوبات المتداولين هناك؟
- حدد منطقة الـ Inducement (الإغراء): أين تم إيهام المتداولين بالدخول قبل أن ينعكس السعر؟
- ابحث عن سيولة الفريمات الكبيرة (HTF Liquidity) التي لم تُلمس بعد.

📈 **المرحلة الثانية: بنية السوق وتدفق الأوامر (Structure & Order Flow)**
- هل الكسر الحالي هو BOS حقيقي (بجسم الشمعة) أم مجرد Liquidity Sweep (بذيل الشمعة)؟
- حدد مرحلة وايكوف الحالية: (تجميع، إعادة تجميع، تصريف، إعادة تصريف).
- تأكد من عدم وجود "انحراف" (Divergence) بين السعر والزخم.

🎯 **المرحلة الثالثة: مصفوفة الدخول الذهبية (Entry Confluence)**
- استخرج الـ Order Block (OB) الذي تسبب في كسر الهيكل بشرط وجود Fair Value Gap (FVG) غير ملموس بجانبه.
- لا تعترف بأي منطقة دخول إذا كانت في منطقة الـ Equilibrium؛ يجب أن يكون الشراء من منطقة الـ Discount والبيع من منطقة الـ Premium.

⚠️ **المرحلة الرابعة: ميزان المخاطر (The Kill Switch)**
- حدد "نقطة بطلان التحليل" (Invalidation Point): السعر الذي إذا وصل إليه السوق، تصبح الفرضية كاملة خاطئة.
- ابحث عن SFP (Swing Failure Pattern): إذا رأيت ذيلاً طويلاً جداً اخترق قمة سابقة، حذر المستخدم فوراً من "فخ بيع".
