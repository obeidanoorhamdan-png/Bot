...
async def handle_photo_analysis(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        prompt = f"""
        [SYSTEM_TASK: TOTAL_MARKET_DECRYPTION_V5_STRICT]
[ROLE]: Hedge Fund Quantitative Strategist & IPDA Logic Expert.
[METHODOLOGY]: Fusion of (SMC + ICT + Wyckoff + Order Flow + Macro Timeframes).

قم بتحليل الشارت المرفق بدقة مجهرية بناءً على البروتوكول التالي:

المرحلة 1: الفلترة الزمنية وسياق السوق (ICT Macro):
- رصد السعر بالنسبة لـ (Midnight Open) و (Kill Zones). هل الحركة الحالية Judas Swing أم توزيع حقيقي؟

المرحلة 2: هندسة السيولة (Liquidity Engineering):
- تحديد مناطق الإغراء (Inducement)، وسيولة المتداولين الأفراد (Retail Trendlines)، ومناطق BSL/SSL التي لم تُستهدف.
- فحص وجود انحراف SMT مع الأصول المرتبطة.

المرحلة 3: هيكل السوق ووايكوف (Market Structure):
- رصد (MSS/CHoCH) الحقيقي الناتج عن (Displacement).
- تصنيف الدورة: (Accumulation, Re-Accumulation, Distribution, Re-Distribution).

المرحلة 4: مصفوفة الـ PD Arrays:
- استخراج مناطق الـ Order Blocks المرتبطة بـ FVG في مناطق (Discount/Premium) حصراً.

المرحلة 5: تأكيد تدفق الأوامر (Order Flow):
- البحث عن علامات الامتصاص (Absorption) واندفاع الزخم المؤسسي (Institutional Sponsorship).
...
- **القرار الفني**: (بيع 🔴 / شراء 🟢 / انتظار 🟡)
- **قوة الإشارة**: (🔥 عالية - تلاقي 3 مدارس / ⚡ متوسطة / ❄️ ضعيفة)
- **نقطة الدخول الذهبية (POI)**: [تحديد السعر بدقة]
- **الأهداف (TPs)**: 
   - 🎯 **TP1**: [هدف قريب - سحب سيولة أولية]
   - 🎯 **TP2**: [هدف متوسط - منطقة عرض/طلب رئيسية]
   - 🎯 **TP3**: [هدف بعيد - امتداد هيكلي أو سيولة كبرى]
- **وقف الخسارة (SL)**: [خلف منطقة الحماية المؤسسية]
- **مستوى الثقة**: % [تقييم منطقي ]
- **المدة المتوقعة**: [حدد عدد الشموع أو الدقائق بدقة للوصول للهدف الأول]
- **الوقت الفعلي للمس الهدف**: [توقع الساعة والدقيقة بناءً على وقت الشارت الحالي]
- **أقصى مدة صلاحية (Expiration)**: [إذا لم يصل السعر خلال X دقيقة، تصبح الصفقة خطرة زمنياً]
- **قوة الإشارة ومستوى الثقة**: (التقييم من 100% + ذكر عدد المدارس المتلاقية)
