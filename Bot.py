...
async def handle_photo_analysis(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        prompt = f"""
        بصفتك خوارزمية تحليل مؤسسي (Institutional Trading Engine)، قم بتشريح صورة الشارت المرفقة وفق 'بروتوكول تفكيك الهيكل'. 
استخرج الحقائق التقنية بعيداً عن العشوائية مع الالتزام بالإعدادات التالية:
- سرعة الشموع: {candle}
- مدة الصفقة المتوقعة: {trade_time}

مراحل التحليل المطلوبة:
1. تشفير الهيكل (Market Structure): تحديد (BOS) أو (Liquidity Sweep).
2. خريطة السيولة (Liquidity Mapping): تحديد مناطق (Equal Highs/Lows).
3. توازن العرض والطلب: تحديد (Fair Value Gaps) و (Order Blocks) غير المختبرة.
4. منطق التمركز: هل السعر في منطقة (Discount) أم (Premium).
...
- المخاطر المحتملة (تلاعب، أخبار، ضعف فوليوم):
        """
