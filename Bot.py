...
MISTRAL_MODEL_AUDIT = "mistral-large-latest"    # للتدقيق والتحليل النقدي
...
def home():
    ...
    <body>
        ...
        <p>Obeida Trading - (Enhanced Dual Model System)</p>
    </body>
    </html>
    """
...
def get_round_number_distance(current_price):
    """حساب المسافة من أقرب رقم مستدير"""
    try:
        next_round_up = math.ceil(current_price * 1000) / 1000  # أقرب .000 للأعلى
        next_round_down = math.floor(current_price * 1000) / 1000  # أقرب .000 للأسفل
        
        distance_to_up = abs(next_round_up - current_price)
        distance_to_down = abs(current_price - next_round_down)
        
        closest_distance = min(distance_to_up, distance_to_down)
        closest_round = next_round_up if distance_to_up < distance_to_down else next_round_down
        
        return closest_distance * 10000, closest_round  # إرجاع المسافة بالنقاط والرقم المستدير
    except:
        return 999, 0  # في حالة خطأ، إرجاع مسافة كبيرة
...
3. حاجز السيولة: لا دخول مع FVG غير مغطاة في الاتجاه المعاكس
4. جدار الأرقام المستديرة: ممنوع تماماً إعطاء إشارة بيع عندما يبعد السعر أقل من 10 نقاط عن رقم مستدير (.000 أو .500)
...
LAST MINUTE RULE: تجاهل الانعكاسات في الدقيقة 59/29

🧠 نظام الذكاء التحليلي المتكامل

🎲 مصفوفة القرار الذكية المعدلة:
🚀 [ ] قوة الزخم الحالي: 
• إذا كانت آخر 3 شموع ماروبوزو (≥90% إمتلاء) = تلقائياً 10 نقاط × 2 = 20 نقطة
• إذا كانت الشموع صاعدة مع إغلاق أعلى من الفتحة = 9 نقاط × 2 = 18 نقطة
• إذا كانت هناك 2 شمعة ماروبوزو من 3 = 8 نقاط × 2 = 16 نقطة
• أقل من ذلك = احسب بشكل طبيعي (1-7) بدون ضرب

🎯 **القاعدة الجديدة**: إذا حصل الزخم على 18-20 نقطة، يلغي أي تأثير سلبي من مؤشرات الدايفرجنس أو الهيكل الهابط.

[ ] اتجاه الهيكل الأساسي: صاعد قوي (9-10) | صاعد ضعيف (6-8) | جانبي (4-5) | هابط ضعيف (2-3) | هابط قوي (0-1)
[ ] حجم التداول النسبي: كبير جداً (9-10) | كبير (6-8) | متوسط (4-5) | صغير (2-3) | معدوم (0-1)
[ ] توافق الإطارات الزمنية: توافق كامل (9-10) | توافق جزئي (6-8) | تعادل (4-5) | تضارب جزئي (2-3) | تضارب كلي (0-1)
[ ] جودة نمط الشموع: نموذج مثالي (9-10) | نموذج جيد (6-8) | غير واضح (4-5) | نموذج ضعيف (2-3) | لا نموذج (0-1)
[ ] قوة مستويات S/R: مستويات قوية (9-10) | مستويات جيدة (6-8) | مستويات ضعيفة (4-5) | لا مستويات (2-3) | اختراق كامل (0-1)
[ ] تأثير السياق الزمني: توقيت مثالي (9-10) | توقيت جيد (6-8) | توقيت عادي (4-5) | توقيت سيء (2-3) | توقيت خطير (0-1)
[ ] عوامل خارجية مؤثرة: ظروف مثالية (9-10) | ظروف جيدة (6-8) | ظروف محايدة (4-5) | ظروف سيئة (2-3) | ظروف خطيرة (0-1)

📈 حساب النتيجة النهائية: (مجموع النقاط المعدلة / 90 × 100)%
ملاحظة: نقطة "قوة الزخم الحالي" تضرب في 2 إذا كانت 8-10.

🛑 **حظر البيع المطلق عند الأرقام المستديرة:**
"يُمنع منعاً باتاً إصدار إشارة بيع إذا كان السعر الحالي يبعد أقل من 10 نقاط عن رقم مستدير (.000 أو .500) وكانت آخر شمعة أو شمعتين صاعدتين. يجب الانتظار حتى يلامس السعر الرقم أولاً، لأن OTC تستهدف هذه المناطق لسحب السيولة."
...
"في سوق OTC، إذا كانت الشمعة الحالية هي آخر دقيقة في إغلاق ساعة أو نصف ساعة (مثل الدقيقة 59 أو 29)، يتم تجاهل إشارات الانعكاس تماماً، والأولوية لاستمرار الزخم (Momentum Continuity) لأن المنصات تضخ سيولة لضرب مناطق الـ Stop Loss عند الإغلاقات الكبرى."
...
"إذا تعارضت منطقة العرض مع شمعة اندفاعية (Marubozu) تخترق مستويات السيولة، اعتبر المنطقة 'مكسورة' فوراً ولا تقترح البيع إلا بعد إعادة اختراق ناجحة أو فشل اختراق مؤكد (SFP) مع تأكيد من شمعتين."
...
"""
        ...
        # برومبت التدقيق المحدث والمحسن
        AUDIT_PROMPT = f"""
🚨 **أنت الآن مدقق تقني متشكك - مهمتك كشف الأخطاء وليس تأكيدها** 🚨

📋 **التحليل الأولي المستلم للتدقيق:**
{initial_analysis}

🎯 **القواعد الحاسمة للمدقق (يجب تطبيقها حرفياً):**

1. **منع التحيز التأكيدي**: لا توافق تلقائياً على التحليل الأولي. ابحث عن الأخطاء بنظرة نقدية.

2. **حظر بيع الأرقام المستديرة المطلق**: 
   - إذا كان السعر يبعد أقل من 10 نقاط عن رقم مستدير (.000/.500)
   - وكانت آخر شمعة أو شمعتين صاعدتين
   - يجب رفض أي إشارة بيع تماماً والانتظار حتى يلامس السعر الرقم أولاً

3. **أولوية الزخم القصير على الهيكل الطويل**:
   - إذا رأيت 3 شموع متتالية قوية (Marubozu ≥90% إمتلاء) باتجاه رقم مستدير
   - ممنوع من تأكيد أي إشارة بيع حتى لو كان الهيكل العام هابطاً
   - الزخم اللحظي هو "القائد" في فريم M1 والمتوسط

4. **البحث الإلزامي عن هذه الأخطاء**:
   - [ ] زخم قوي (≥3 شموع ماروبوزو) يتم تجاهله
   - [ ] بيع قرب رقم مستدير بدون لمسه أولاً
   - [ ] عدم مراعاة جلسة السوق الحالية ({session_name})
   - [ ] خطأ في قراءة الأسعار من المحور اليمني
   - [ ] تعارض بين فريم التحقق ({verification_timeframe}) والفريم الرئيسي ({candle})

🔍 **كيفية التدقيق بشكل صحيح:**

1. **تحقق من الأسعار الدقيقة**: قارن كل سعر مذكور في التحليل مع ما تراه في الصورة. إذا كان هناك فرق >5 نقاط، صححه.

2. **تطبيق فلتر الأرقام المستديرة الحاسم**: 
   - استخدم دالة المسافة: get_round_number_distance()
   - إذا كانت المسافة <10 نقاط والاتجاه صاعد = منع البيع

3. **تقييم قوة الزخم الحقيقي**:
   - 3 شموع ماروبوزو صاعدة = زخم قوي جداً (20 نقطة)
   - 2 شموع ماروبوزو من 3 = زخم قوي (16 نقطة)
   - الزخم القوي يلغي تأثير الهيكل الهابط

4. **السياق الزمني الحاسم**:
   - جلسة: {session_name} ({session_time})
   - سيولة: {session_vol}
   - تأثير أخبار: {news_impact} (مضاعف مخاطر: ×{news_risk_multiplier})

5. **فريم التحقق الصحيح**: 
   - فريم التحقق: {verification_timeframe}
   - لا تقبل كسر هيكل إلا إذا كان واضحاً على هذا الفريم
   - تجاهل الذيول الصغيرة كسحب سيولة وليس كسر هيكل

📊 **نظام تسجيل الأخطاء**:

❌ **أخطاء فادحة (تتطلب تصحيح فوري)**:
1. بيع عند رقم مستدير قريب (<10 نقاط) مع زخم صاعد
2. تجاهل 3 شموع ماروبوزو متتالية
3. قراءة خاطئة للأسعار (>5 نقاط فرق)
4. توصية ضد جلسة السيولة العالية ({session_name})

⚠️ **أخطاء متوسطة**:
1. عدم مراعاة فريم التحقق ({verification_timeframe})
2. تجاهل فلتر الأخبار ({news_warning if news_warning else 'لا أخبار'})
3. تقدير خاطئ لقوة الإشارة

✅ **ما يجب أن يفعله المدقق الذكي**:
1. ابدأ بـ "🔍 **تدقيق تقني نقدي**:" كعنوان
2. اذكر عدد الأخطاء التي وجدتها
3. صحح كل خطأ بدقة
4. قدم البديل الصحيح مع التبرير
5. تأكد من تطبيق جميع القواعد الحاسمة

📈 **المعطيات الفنية الحالية**:
• الإطار الحالي: {candle} ({candle_category})
• فريم التحقق: {verification_timeframe} (للكسر الهيكلي)
• استراتيجية التداول: {trading_strategy}
• جلسة السوق: {session_name} ({session_time})
• حالة السيولة: {session_vol}
...
🔍 **تدقيق تقني نقدي**:
• الأخطاء المكتشفة: [عدد] خطأ (فادح/متوسط/طفيف)
• التحليل الأولي: [✅ مقبول / ⚠️ يحتاج تصحيح / ❌ مرفوض]

📊 *التحليل الفني المصحح*:
• البصمة الزمنية: {kill_zone_status}
• حالة الهيكل: (صاعد/هابط) + (مرحلة وايكوف الحالية) + (توافق 4/4 إطارات: نعم/لا)
• خريطة السيولة: (أقرب فخ سيولة Inducement + مناطق السيولة المستهدفة)
• الفجوات السعرية: (المناطق التي سيعود السعر لتغطيتها)

🎯 *الإشارة التنفيذية المصححة*:
• مقارنة مع التحليل السابق: [✅ مطابق تماماً / ⚡ محسّن / ❌ مصحح]، درجة التشابه: [0–100]%
• السعر الحالي: [السعر الدقيق من الشارت بعد التصحيح]
• حالة الشمعة: [مفتوحة / مغلقة]
• القرار الفني: (شراء 🟢 / بيع 🔴 / احتفاظ 🟡)
• قوة الإشارة: (عالية جدا 💥 / عالية 🔥 / متوسطة ⚡ / ضعيفة ❄️)
• نقطة الدخول: [السعر الدقيق بناءً على OB + شرط الإغلاق]
...
⚠️ *إدارة المخاطر المصححة*:
• مستوى الثقة: [0-100]٪ = [💥/🔥/⚡/❄️/🚫]
• نقطة الإلغاء: [السعر الذي يفسد التحليل]
• فريم التحقق: {verification_timeframe} (للتأكد من كسر الهيكل الحقيقي)

📝 **ملاحظات المدقق**:
• الأخطاء التي تم تصحيحها: [1. ... 2. ...]
• القواعد التي تم تطبيقها: [1. ... 2. ...]
• السبب الرئيسي لأي تغيير: [توضيح]

**تذكر**: مهمتك هي كشف الأخطاء وتصحيحها، ليس تأكيد التحليل الأولي. كن ناقداً وموضوعياً.
"""
        ...
        full_result = (
            ...
            f"🏛️ **الجلسة:** {session_name} ({session_time})\n"
            f"📈 **الاستراتيجية:** {trading_strategy}\n"
            f"━━━━━━━━━━━━━━━━\n"
            f"{audit_result}\n\n"
            ...
            f"• تأثير الأخبار: {news_impact} (مضاعف: ×{news_risk_multiplier})\n"
            f"━━━━━━━━━━━━━━━━━\n"
            f"🤖 **Powered by - Obeida Trading (النظام المزدوج المحسن)**"
        )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "• إعدادات تخصيص كاملة\n"
        "• تحليل دقيق بالأرقام\n\n"
        "📡 **نظام التحليل المزدوج المحسن:**\n"
        f"1. التحليل الأولي ({MISTRAL_MODEL})\n"
        f"2. التدقيق النقدي ({MISTRAL_MODEL_AUDIT})\n"
        f"3. فلاتر ذكية لمنع الأخطاء\n\n"
        "🔒 **نظام الحماية الجديد:**\n"
        "• منع البيع عند الأرقام المستديرة\n"
        "• أولوية الزخم على الهيكل\n"
        "• فريم تحقق ديناميكي",
        reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=False),
        parse_mode="Markdown"
    )
    ...
    if user_message == "⚙️ إعدادات التحليل":
        ...
        if not candle or not trade_time:
            ...
            await update.message.reply_text(
                ...
                f"• {time_display}\n\n"
                f"📡 **نظام التحليل المزدوج المحسن:**\n"
                f"1. التحليل الأولي ({MISTRAL_MODEL})\n"
                f"2. التدقيق النقدي ({MISTRAL_MODEL_AUDIT})\n\n"
                f"🔒 **نظام الحماية الجديد:**\n"
                f"• منع البيع عند الأرقام المستديرة\n"
                f"• أولوية الزخم على الهيكل\n"
                f"• فريم تحقق ديناميكي\n\n"
...
async def handle_settings_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_message in TRADE_TIMES:
        ...
        await update.message.reply_text(
            ...
            f"📡 **نظام التحليل المزدوج المحسن:**\n"
            f"• موديل التحليل: {MISTRAL_MODEL}\n"
            f"• موديل التدقيق: {MISTRAL_MODEL_AUDIT}\n"
            f"• نظام حماية من الأخطاء الشائعة\n\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    📡 **نظام المزدوج المحسن للتحليل:**
    • **المرحلة 1:** التحليل الأولي ({MISTRAL_MODEL})
    • **المرحلة 2:** التدقيق النقدي ({MISTRAL_MODEL_AUDIT})
    • **المرحلة 3:** فلاتر ذكية لمنع الأخطاء
...
def run_telegram_bot():
    ...
    print(f"📊 نظام الموديل المزدوج المحسن: {MISTRAL_MODEL} + {MISTRAL_MODEL_AUDIT}")
    ...
    print("🔒 نظام الحماية الجديد مفعل: منع البيع عند الأرقام المستديرة")
...
def main():
    """الدالة الرئيسية"""
    print("🤖 Starting Powered by - Obeida Trading (النظام المزدوج المحسن)...")
    print("=" * 60)
    print(f"📊 موديل التحليل: {MISTRAL_MODEL}")
    print(f"🔍 موديل التدقيق: {MISTRAL_MODEL_AUDIT}")
    print("🔒 نظام الحماية الجديد: منع البيع عند الأرقام المستديرة")
    print("⚡ أولوية الزخم على الهيكل في الفريمات الصغيرة")
    print("=" * 60)
