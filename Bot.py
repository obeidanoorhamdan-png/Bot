...
MISTRAL_MODEL_1 = "pixtral-large-latest"  # النموذج الأول
MISTRAL_MODEL_2 = "mistral-large-pixtral-2411"  # النموذج الثاني
...
def home():
    ...
    <body>
        ...
        <p>AI Provider: Mistral AI (Dual Models)</p>
    </body>
    </html>
    """
...
@app.route('/health')
def health():
    return {"status": "active", "ai_provider": "Mistral AI", "model": MISTRAL_MODEL_1, "timestamp": time.time()}
...
# --- النظام الجديد: دمج نموذجين للتحليل ---
def analyze_with_dual_models(prompt, image_base64=None, is_chat=False, user_message=""):
    """تحليل باستخدام نموذجين من Mistral"""
    
    def call_mistral_model(model, messages, max_tokens=1500, temperature=0.7):
        """استدعاء نموذج Mistral"""
        headers = {
            "Authorization": f"Bearer {MISTRAL_KEY}",
            "Content-Type": "application/json"
        }
    ...
    if image_base64:
        ...
        # رسالة للنموذج الثاني (بنفس الصورة)
        messages_2 = messages_1.copy()
        
        # استدعاء النموذجين بشكل متوازي
        result_1 = call_mistral_model(MISTRAL_MODEL_1, messages_1, max_tokens=2000, temperature=0.15)
        result_2 = call_mistral_model(MISTRAL_MODEL_2, messages_2, max_tokens=1800, temperature=0.2)
        
        # دمج النتائج
        if result_1 and result_2:
            return merge_analysis_results(result_1, result_2, "صورة")
        elif result_1:
            return result_1
        elif result_2:
            return result_2
        else:
            return "❌ فشل في التحليل باستخدام النموذجين"
    ...
    # إذا كان دردشة
        ...
        result_1 = call_mistral_model(MISTRAL_MODEL_1, messages_1, max_tokens=1200, temperature=0.7)
        result_2 = call_mistral_model(MISTRAL_MODEL_2, messages_2, max_tokens=1000, temperature=0.8)
        
        # دمج النتائج للدردشة
        if result_1 and result_2:
            return merge_chat_results(result_1, result_2)
        elif result_1:
            return result_1
        elif result_2:
            return result_2
        else:
            return "❌ فشل في الرد باستخدام النموذجين"
    
    return None

def merge_analysis_results(result_1, result_2, analysis_type):
    """دمج نتائج تحليل النموذجين"""
    
    # إذا كانت النتائج متشابهة، نأخذ الأفضل
    if result_1 and result_2:
        # اختر النتيجة الأطول (عادةً أكثر تفصيلاً)
        if len(result_1) > len(result_2):
            final_result = result_1
            source = MISTRAL_MODEL_1
        else:
            final_result = result_2
            source = MISTRAL_MODEL_2
        
        # إضافة تذييل يوضح أن التحليل من نموذجين
        footer = f"\n\n━━━━━━━━━━━━━━━━━━\n🤖 **تحليل مزدوج بقوة نموذجين:**\n• {MISTRAL_MODEL_1}\n• {MISTRAL_MODEL_2}\n━━━━━━━━━━━━━━━━━━"
        
        return final_result + footer
    
    elif result_1:
        return result_1 + f"\n\n🤖 **النموذج المستخدم:** {MISTRAL_MODEL_1}"
    
    elif result_2:
        return result_2 + f"\n\n🤖 **النموذج المستخدم:** {MISTRAL_MODEL_2}"
    
    return "❌ لم يتم الحصول على تحليل من النماذج"

def merge_chat_results(result_1, result_2):
    """دمج نتائج الدردشة من النموذجين"""
    
    if not result_1 and not result_2:
        return "عذراً، لم أتمكن من معالجة سؤالك. يرجى المحاولة مرة أخرى."
    ...
    # إذا كان لدينا نتائج من كلا النموذجين
    # نأخذ الأفضل بناءً على الجودة
    quality_1 = calculate_response_quality(result_1)
    quality_2 = calculate_response_quality(result_2)
...
def calculate_response_quality(response):
    ...
    # الطول (رد أطول عادة أفضل)
    quality_score += min(len(response) / 100, 10)
    
    # التنظيم (وجود عناوين)
    if '**' in response or '•' in response or '🔹' in response:
        quality_score += 5
    
    # المعلوماتية (كلمات مفتاحية)
    info_keywords = ['نصيحة', 'توصية', 'شرح', 'تفصيل', 'مثال', 'خطوات', 'نقاط']
...
async def start_chat_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        ...
        text="🚀 **وضع الدردشة Obeida Trading**\n\n"
             ...
             "• كاتب محتوى إبداعي\n"
             "• مساعد شخصي ذكي\n\n"
             "🤖 **مميزات جديدة:** تحليل مزدوج باستخدام نموذجين متقدمين\n\n"
...
**تذكر جيداً:** أنت Obeida Trading، المساعد الذكي الذي يحول التعقيد إلى بساطة، ويمنحك دائماً أكثر مما تطلب!""")
    ...
    try:
        # استدعاء نظام النموذجين المزدوج للدردشة
        result = analyze_with_dual_models(selected_prompt, is_chat=True, user_message=user_message)
        
        # تنظيف النص من التكرارات
        result = clean_repeated_text(result)
        
        # إضافة تذييل مميز
        footer = "\n\n━━━━━━━━━━━━━━━━━━\n🤖 **Obeida Trading** - تحليل مزدوج باستخدام نموذجين 🤖"
...
"""
        ...
        # 🚀 استدعاء النظام المزدوج لتحليل الصور
        await wait_msg.edit_text("🧠 جاري تحليل الشارت باستخدام نموذجين متقدمين...")
        
        result = analyze_with_dual_models(prompt, image_base64=base64_img)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "• 🆕 **نظام مزدوج: {MISTRAL_MODEL_1} + {MISTRAL_MODEL_2}**\n"
        "• 🧠 دردشة ذكية مع مخرجات محسنة\n"
        "• 📈 نظام توصيات جاهزة\n"
...
async def handle_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_message == "⚙️ إعدادات التحليل":
        ...
        if not candle or not trade_time:
            ...
            await update.message.reply_text(
                f"📊 **جاهز للتحليل المزدوج**\n\n"
                f"الإعدادات الحالية:\n"
                f"• سرعة الشموع: {candle}\n"
                f"• {time_display}\n\n"
                f"🤖 **النماذج المستخدمة:**\n"
                f"• {MISTRAL_MODEL_1} (تحليل فني)\n"
                f"• {MISTRAL_MODEL_2} (تحليل استراتيجي)\n\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    🎯 **نظام النماذج المزدوج:**
    • **{MISTRAL_MODEL_1}:** تحليل فني متقدم
    • **{MISTRAL_MODEL_2}:** تحليل استراتيجي عميق
    • **دمج النتائج:** تقارير شاملة بدقة عالية
...
def run_telegram_bot():
    ...
    print(f"🤖 Dual Model System: {MISTRAL_MODEL_1} + {MISTRAL_MODEL_2}")
