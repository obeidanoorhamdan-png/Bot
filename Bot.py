...
- فحص حركة السعر الحالية وتحديد علاقتها بفترات السيولة (توقيت لندن/نيويورك). هل نحن في منطقة تلاعب (Judas Swing) أم توزيع حقيقي؟

المرحلة 2: هندسة السيولة (Liquidity Engineering):
- حدد بوضوح مناطق الإغراء (Inducement)، وسيولة المتداولين الأفراد (Retail Liquidity)، ومناطق BSL/SSL التي لم تُستهدف بعد.

المرحلة 3: هيكل السوق ووايكوف (Market Structure):
- حدد كسر الهيكل (BOS) وتغير الاتجاه (CHoCH).
- صنّف المرحلة الحالية وفق وايكوف (تجميع، إعادة تجميع، تصريف، إعادة تصريف).

المرحلة 4: مناطق العرض والطلب (PD Arrays):
- استخرج مناطق الـ Order Blocks المرتبطة بفجوات القيمة العادلة (FVG) الواقعة في مناطق الخصم أو العودة (Discount/Premium).

المرحلة 5: تأكيد تدفق الأوامر (Order Flow):
- البحث عن علامات امتصاص السيولة (Absorption) واندفاع الزخم المؤسسي.
