...
async def handle_photo_analysis(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # برومبت آمن للتحليل الفني - تم تعديله لمنع التكرار
                prompt = f"""
                        [SYSTEM_TASK: TOTAL_MARKET_DECRYPTION_V9_THE_MASTER_PROTOCOL]
بصفتك "الخوارزمية السيادية" لإدارة محافظ صناديق التحوط، قم بتشريح الشارت المرفق عبر دمج (SMC + ICT + Wyckoff) مع تفعيل بروتوكول "صفر تسامح" مع التلاعب:

⚠️ **القوانين الإجبارية القاطعة (Protocol Zero):**
1. **قانون الجهد والنتيجة المؤسسي**: حلل الزخم؛ إذا كانت الشمعة ضخمة ولم تكسر هيكلاً (BOS)، فهي "فخ استنفاذ". إذا كانت صغيرة وكسرت هيكلاً، فهذا "تجميع خفي".
2. **قانون التوافق الرقمي والفركتلي**: يجب مطابقة السعر الحالي مع ذيول الشموع في الفريمات الأصغر لضمان عدم وجود انزلاق وهمي (Slippage) في منصة الـ OTC.
3. **قانون السيولة المزدوجة والتحريض**: لا دخول إلا بعد: (1) سحب سيولة Sweep، (2) ظهور فخ Inducement للمتداولين، (3) ارتداد يترك فجوة FVG واضحة.
4. **قانون النطاق الذهبي (OTE)**: الدخول حصراً في مستويات التصحيح العميقة (0.618 - 0.786) لضمان أعلى RR وأقل انعكاس.

🔬 **الإجراء التحليلي الكامل (16 مرحلة احترافية):**

[المرحلة 1-4: الهندسة الرقمية والهيكلية]
- التحقق الرقمي، التفكيك الهيكلي، التوافق الفركتلي مع الفريمات الأكبر، وتحديد اتجاه تدفق الأوامر (Order Flow).

[المرحلة 5-8: تشريح السيولة وكشف التلاعب]
- رسم خرائط السيولة (EQL/EQH)، رصد سيولة التحريض (Induced Liquidity)، تمييز الكسر الحقيقي عن الـ Fake Sweep، وتحليل الجهد مقابل النتيجة.

[المرحلة 9-12: الهندسة السعرية ومناطق القوة]
- بناء مناطق العرض والطلب، تحديد الـ Order Block النشط، فحص الفجوات السعرية (FVG/Inversion)، وتحديد مستويات الـ OTE.

[المرحلة 13-16: مصفوفة التنفيذ والقتل]
- مصفوفة الدخول الذهبية، النمذجة الزمنية، تفعيل الـ Kill Switch (نقطة الإبطال)، وفك شفرة الذيول (Wick Cryptanalysis).

المعطيات التقنية:
- إطار الشمعة : (Timeframe): {candle}
- وقت التداول المحدد : {time_for_prompt}

قدم التحليل باللغة العربية وبالتنسيق التالي (بدون تكرار):
