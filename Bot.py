...
async def handle_photo_analysis(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        prompt = f"""
        بصفتك خوارزمية تحليل مؤسسي (Institutional Trading Engine)، قم بتشريح صورة الشارت المرفقة وفق 'بروتوكول تفكيك الهيكل'. المطلوب هو استخراج الحقائق التقنية بعيداً عن العشوائية.
​مراحل التحليل المطلوبة:
​تشفير الهيكل (Market Structure): حدد الاتجاه (صاعد/هابط) وهل الكسر الأخير كان (BOS) حقيقياً أم سحب سيولة (Liquidity Sweep) بذيول الشموع؟
​خريطة السيولة (Liquidity Mapping): حدد مناطق 'السيولة المفتوحة' (Equal Highs/Lows) التي ينجذب إليها السعر كأهداف قادمة.
​توازن العرض والطلب: حدد مناطق الفجوات السعرية (Fair Value Gaps) وكتل الأوامر (Order Blocks) التي لم يتم اختبارها.
​منطق التمركز: حدد هل السعر حالياً في منطقة 'Discount' (رخيص للشراء) أم 'Premium' (غالٍ للبيع).
​قدم النتيجة باختصار شديد وفق التنسيق التالي حصراً:
​📊 التحليل الفني:
​النمط السائد:
​مستويات الدعم/المقاومة:
​🎯 التوقع:
​الإتجاه: ( صعود ⬆️ / نزول ⬇️ / ثابت ➡️ )
​توقع الإتجاه: (صعود/نزول: عالي، متوسط، منخفض)
​التوصية: ( بيع 🔴 / شراء 🟢 / إحتفاظ 🟡 )
​السعر الحالي:
​الهدف (TP):
​الوقف (SL):
​مستوى الثقة: XX٪ (بناءً على تلاقي 3 أدلة)
​نقطة الدخول المقترحة:
​نقطة الوصول المتوقعة:
​⚠️ التحذيرات والمخاطر:
​نقطة بطلان التحليل (Invalidation Point):
​المخاطر المحتملة (تلاعب OTC، أخبار، ضعف فوليوم):
        """
