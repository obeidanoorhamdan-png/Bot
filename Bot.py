...
async def handle_photo_analysis(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        prompt = f"""
[SYSTEM_TASK: REAL_CAPITAL_EXECUTION_V1]
🔥 **صفقة برأس مال حقيقي - تنفيذ مباشر** 🔥

بصفتك خبير استراتيجيات في صناديق التحوط، ومتخصص في مدارس (SMC + ICT + Wyckoff + Order Flow)، قم بتحليل الشارت المرفق لصفقة تنفيذية فورية.

💰 **معطيات الصفقة الحقيقية**:
- نوع الحساب: REAL MONEY
- الإطار الزمني: {candle}
- وقت التنفيذ: {time_for_prompt}
- الهدف: صفقة واحدة فورية

🔍 **المرحلة 1: الفحص السريع للفرصة**:
- الوضع الزمني: (داخل/خارج Kill Zone) - مع تحديد التوقيت
- حالة السوق: (تلاعب Judas Swing / حركة حقيقية)

🔍 **المرحلة 2: السيولة القابلة للاستهداف الآن**:
- أقرب Inducement: (السعر والمسافة)
- مناطق BSL/SSL غير المسحوبة: (الأولوية للقريبة)

🔍 **المرحلة 3: الهيكل الفوري**:
- آخر BOS: (التأكيد على قربها)
- مرحلة وايكوف الحالية: (وضوح المرحلة)

🔍 **المرحلة 4: مناطق الدخول الفوري**:
- Order Block نشط الآن: (أقرب منطقة)
- FVG يحتاج ملء: (الأكثر ضغطاً)

🔍 **المرحلة 5: تأكيد التدفق الحالي**:
- علامات Absorption حالية: (نعم/لا)
- زخم مؤسسي مباشر: (اتجاهه)

🎯 **التحليل التنفيذي (جاهز للدخول)**:

📊 **الوضع الحالي**:
- **البصمة الزمنية**: [داخل/خارج Kill Zone] - السبب: [______]
- **حالة الهيكل**: [صاعد/هابط] + [مرحلة وايكوف] - التأكيد: [______]
- **خريطة السيولة**: 
  أقرب فخ: [______] 
  مستهدفة: [______]
- **الفجوات النشطة**: 
...
⚡ **التوصية الفورية**:
- **الاتجاه القادم**: ⬆️ صعود / ⬇️ نزول / ➡️ تذبذب
- **القرار الفني**: 🟢 شراء / 🔴 بيع / 🟡 انتظار
- **قوة الإشارة**: 🔥 عالية / ⚡ متوسطة / ❄️ ضعيفة
- **نقطة الدخول (POI)**: [السعر بالضبط: ______]
- **الأهداف التنفيذية**:
   🎯 **TP1**: [______] (أقرب سيولة)
   🎯 **TP2**: [______] (منطقة رئيسية)  
   🎯 **TP3**: [______] (سيولة كبرى)
- **وقف الخسارة (SL)**: [______] (خلف مستوى: ______)
- **مستوى الثقة**: [__]%
- **الجدول الزمني**:
  📅 البدء: خلال [__] شمعة
  ⏰ الوصول لـ TP1: الساعة [__:__]
  ⏳ الصلاحية: حتى [وقت الانتهاء]
- **القوة التقنية**: [__]% (تلاقي [__] مدرسة)
