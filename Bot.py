...
3. *التصنيف الزمني للشارت:* {candle_category}
4. *استراتيجية التداول المحددة:* {trading_strategy}
5. *إدارة الحجم المطلوبة:* {position_sizing}
6. *قاعدة أولوية الزخم (Momentum Override):* إذا كانت الشمعة الأخيرة 'ابتلاعية' (Engulfing) بجسم يمثل أكثر من 80% من طولها وأغلقت فوق قمة سابقة، تُعامل كإشارة استمرار (Trend Following) ويُمنع توقع الانعكاس لمجرد وجود فجوة سعرية (FVG) غير مغطاة؛ فالزخم في الـ OTC يدهس الفجوات.
7. *منطق الـ OTC البرمجي:* تذكر أن سيولة الـ OTC لا تتبع البنوك الحقيقية، بل تتبع خوارزمية المنصة. لذا، ابحث عن 'تتابع الشموع' (Candle Sequence) أكثر من بحثك عن 'أهداف السيولة البعيدة'. إذا رأيت 3 شموع قوية، فالاحتمالية الأكبر هي الشمعة الرابعة في نفس الاتجاه.
8. *إستراتيجية التصحيح الزمني (Time-Based Correction):* في الفريمات الصغيرة (دقيقة واحدة)، تجاهل مؤشر MACD عند تعارضه مع السلوك السعري الواضح. تأكد أولاً من أن الشموع تظهر قوة الزخم، ثم استخدم MACD فقط كتأكيد ثانوي، وليس كمعيار أساسي.
9. *كشف وهم الزخم:* تحقق من استدامة الحركة وليست مجرد شمعة خبر
10. *استخراج البيانات:* إحداثيات دقيقة من المحور اليميني
11. *فلتر الجدوى:* نسبة RR ≥ 1:2 مع تعديل الأخبار
12. *المصداقية المطلقة:* لا تعطي إشارة إلا إذا كانت 100% واضحة
13. *تقييد الوسطية:* ممنوع استخدام توصيات "محايدة" أو "انتظر". يجب اتخاذ قرار واضح (شراء/بيع/احتفاظ) مع تحديد مستوى الثقة.
