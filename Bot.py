...
"""
        ...
        if response.status_code == 200:
            ...
            # إضافة معلومات الرأس
            analysis_header = f"""📈 **تحليل فني متقدم** - Obeida Trading Pro V9
...
{'⚠️ **تحذير:** تداول بحذر بسبب قرب وقت أخبار' if news_impact != '🟢 منخفض' else '✅ الوضع آمن من الأخبار'}
...
═══════════════════════════════
🤖 **Obeida Trading - نظام التحليل المتكامل**
• SMC + الدرع الأساسي + كشف الزخم الوهمي
• فلتر التلاقي الثلاثي + مصفاة التسعير (PD Array)
"""
...
• 🔍 **كشف الزخم:** خوارزمية كشف الوهم
• 📊 **مدرسة SMC:** مع PD Array
• ⚡ **فلتر التلاقي:** 3/3 شروط
• 🎯 **التنسيق:** محافظ عليه

🚀 **جاهز لتحليل صورة أخرى أو العودة للقائمة الرئيسية.**
...
📊 **اختر الإجراء التالي:**"""
            ...
            error_message = f"""❌ **خطأ في الاتصال بالخادم**

**رمز الخطأ:** {response.status_code}
**السبب:** {response.text[:200] if response.text else 'غير معروف'}

**حلول مقترحة:**
1. تحقق من اتصالك بالإنترنت
2. حاول مرة أخرى بعد قليل
3. تأكد من وضوح الصورة
4. قلص حجم الصورة إذا كانت كبيرة"""
        ...
        keyboard = [["الرجوع للقائمة الرئيسية"]]
        timeout_message = """⏱️ **تجاوز الوقت المحدد**

التحليل استغرق وقتاً أطول من المتوقع.

**نصائح للتحسين:**
1. استخدم صورة أقل تعقيداً
2. تأكد من وضوح الشموع والأرقام
3. قلص منطقة التحليل (Zoom In)
4. حاول مرة أخرى"""
        ...
        print(f"خطأ في تحليل الصورة: {str(e)[:200]}")
        keyboard = [["الرجوع للقائمة الرئيسية"]]
        
        error_type = type(e).__name__
        error_details = str(e)[:150]
        
        error_response = f"""⚠️ **حدث خطأ غير متوقع**
