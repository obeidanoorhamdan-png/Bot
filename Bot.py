...
async def analyze_with_logic_model(text_prompt, image_base64=None):
    """استخدام موديل التحليل المنطقي لاتخاذ القرار"""
    headers = {"Authorization": f"Bearer {MISTRAL_KEY}", "Content-Type": "application/json"}
    
    messages = [{"role": "user", "content": text_prompt}]
    
    # إذا كان هناك صورة، أضفها للرسالة
    if image_base64:
        messages[0] = {
            "role": "user",
            "content": [
...
📋 **تنسيق التلخيص المطلوب:**

⚡ **التنبيهات الفورية:**
• [أهم إشارة فورية]
• [الحركة المتوقعة القادمة]
• [الخطر الأكبر حالياً]

🚀 **الإجراءات المطلوبة:**
• [الإجراء 1: الدخول/الخروج/الانتظار]
• [الإجراء 2: تعديل الأوامر إن وجدت]
• [الإجراء 3: مراقبة مستوى معين]
...
# --- كود تحليل الصور بنظام الموديلات الثلاثة مع البرومبت الكامل ---
async def handle_photo_analysis(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالجة الصور للتحليل الفني المتقدم بنظام الموديلات الثلاثة مع البرومبت الكامل"""
    user_id = update.effective_user.id
    ...
    try:
        ...
        🎯 **استخراج البيانات الأساسية بدقة:**
        1. قراءة القيم الرقمية من المحور العمودي (السعر) بدقة
        2. قراءة التواريخ/الأوقات من المحور الأفقي
        3. استخراج مستويات الدعم والمقاومة الرئيسية
        4. تحديد نقاط التحول (Swing Highs/Lows)
        5. قياس أطوال الشموع وأجسامها بدقة
        6. قراءة أحجام التداول إذا كانت موجودة
        7. تحديد الترند العام (صاعد/هابط/جانبي)
        8. تسجيل مناطق التجميع (Accumulation) والتوزيع (Distribution)
        9. اكتشاف الفجوات السعرية (Gaps)
        
        📊 **معلومات الشارت:**
        - إطار الزمن: {candle}
        - وقت التحليل: {current_time.strftime('%Y-%m-%d %H:%M')}
        
        📝 **المطلوب منك (التحليل البصري فقط):**
        قدم تقريراً مفصلاً يحتوي على:
        
        1. **البيانات الرقمية الدقيقة:**
           - السعر الحالي: [رقم دقيق من المحور]
           - أعلى سعر مرئي: [رقم دقيق]
           - أدنى سعر مرئي: [رقم دقيق]
           - مستويات الدعم الرئيسية: [قائمة بالأرقام]
           - مستويات المقاومة الرئيسية: [قائمة بالأرقام]
           - أقرب رقم مستدير: (.000 أو .500 الأقرب)
        
        2. **التحليل البصري التفصيلي:**
           - نوع الشموع السائدة (طويلة الذيل، أجسام كبيرة، دوجي)
           - نمط الشموع الحالي (إن وجد)
           - كثافة التداول (من خلال حجم الشموع)
           - مناطق الضغط السعري (Price Compression)
           - مناطق الفراغ السعري (Price Void)
        
        3. **المعلومات الزمنية:**
           - الوقت المتبقي لإغلاق الشمعة الحالية
           - حالة الشمعة الحالية (مفتوحة/مغلقة)
           - وقت آخر إغلاق مرئي
        
        **تذكر:** أنت موديل الرؤية، مهمتك استخراج البيانات البصرية فقط، ليس اتخاذ القرارات.
        """
...
4. جدار الأرقام: منع الدخول عند .000/.500 ±5 نقاط بدون CHoCh على {verification_timeframe}
...
🎲 مصفوفة القرار الذكية (أعط نقاط 1-10 لكل بند):
🚀 [ ] قوة الزخم الحالي: اندفاع قوي (9-10) | اندفاع متوسط (6-8) | توازن (4-5) | ضعف متوسط (2-3) | ضعف قوي (0-1)
ملاحظة: إذا حصل هذا البند على 9-10، تضرب النقطة في 2 وتلغي أي تأثير سلبي من مؤشرات الدايفرجنس أو التشبع.
...
ملاحظة: نقطة "قوة الزخم الحالي" تضرب في 2 إذا كانت 9-10.
...
"""
        ...
        logical_analysis = await analyze_with_logic_model(full_prompt, base64_img)
        ...
        full_result = (
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🔍 **نظام التحليل:**\n"
            f"• 🥇 **Pixtral Large**: استخراج البيانات البصرية الدقيقة\n"
            f"• 🥈 **Mistral Large 3**: التحليل المنطقي المتعمق واتخاذ القرار\n"
            f"• 🥉 **Mistral Medium 3**: التلخيص السريع والتنبيهات الفورية\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📊 **الإعدادات المستخدمة:**\n"
            f"• سرعة الشموع: {candle} ({candle_category})\n"
            f"• استراتيجية: {time_display}\n"
            f"• فريم التحقق: {verification_timeframe} (للتحقق من الكسر الهيكلي)\n"
            f"• الجلسة: {session_name} ({session_vol} سيولة)\n"
            f"• تأثير الأخبار: {news_impact} (معامل ×{news_risk_multiplier})\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    if user_message == "⚙️ إعدادات التحليل":
        ...
        if not candle or not trade_time:
            ...
            await update.message.reply_text(
                ...
                f"🥇 Pixtral Large: استخراج البيانات البصرية الدقيقة\n"
                f"🥈 Mistral Large 3: تحليل منطقي متعمق واتخاذ القرار\n"
                f"🥉 Mistral Medium 3: تلخيص سريع وتنبيهات فورية\n\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    • **🥇 Pixtral Large:** استخراج البيانات البصرية الدقيقة من الشارت
    • **🥈 Mistral Large 3:** تحليل منطقي متعمق واتخاذ القرار
...
def run_telegram_bot():
    ...
    print(f"🥇 {MODEL_VISION} - للرؤية والاستخراج البصري الدقيق")
    print(f"🥈 {MODEL_ANALYSIS} - للتحليل المنطقي المتعمق واتخاذ القرار")
    print(f"🥉 {MODEL_SUMMARY} - للتلخيص السريع والتنبيهات الفورية")
